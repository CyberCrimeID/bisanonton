...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `Bisanonton - ${name} ${seasonDetail?.name || ''}` },
    {
      property: 'og:url',
      content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/season/${params.seasonId}/`,
    },
    { property: 'og:title', content: `Bisanonton - ${name} ${seasonDetail?.name || ''}` },
    { name: 'twitter:title', content: `Bisanonton - ${name} ${seasonDetail?.name || ''}` },
  ];
});
