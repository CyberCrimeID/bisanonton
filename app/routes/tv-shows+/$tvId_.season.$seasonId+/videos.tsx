...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Videos` },
    {
      property: 'og:url',
      content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/season/${params.seasonId}/videos`,
    },
    { property: 'og:title', content: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Videos` },
    { name: 'twitter:title', content: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Videos` },
  ];
});
