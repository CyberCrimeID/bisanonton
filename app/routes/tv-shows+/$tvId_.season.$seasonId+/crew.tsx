...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Crew` },
    {
      property: 'og:url',
      content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/season/${params.seasonId}/crew`,
    },
    { property: 'og:title', content: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Crew` },
    { name: 'twitter:title', content: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Crew` },
  ];
});
