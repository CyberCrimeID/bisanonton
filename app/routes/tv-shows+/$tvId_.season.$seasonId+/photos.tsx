...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Photos` },
    {
      property: 'og:url',
      content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/season/${params.seasonId}/photos`,
    },
    { property: 'og:title', content: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Photos` },
    { name: 'twitter:title', content: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Photos` },
  ];
});
