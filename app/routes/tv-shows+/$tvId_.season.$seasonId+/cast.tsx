...
>(({ params, matches }) => {
  ...
  const { name } = detail || {};
  return [
    { title: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Cast` },
    {
      property: 'og:url',
      content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/season/${params.seasonId}/cast`,
    },
    { property: 'og:title', content: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Cast` },
    { name: 'twitter:title', content: `Bisanonton - ${name} ${seasonDetail?.name || ''} - Cast` },
  ];
});
