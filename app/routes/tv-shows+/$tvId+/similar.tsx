...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `Bisanonton - ${name} - Similar` },
      { property: 'og:title', content: `Bisanonton - ${name} - Similar` },
      {
        property: 'og:url',
        content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/similar`,
      },
      { property: 'twitter:title', content: `Bisanonton - ${name} - Similar` },
    ];
  },
);
