...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `Bisanonton - ${name} - Videos` },
      { property: 'og:title', content: `Bisanonton - ${name} - Videos` },
      {
        property: 'og:url',
        content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/videos`,
      },
      { property: 'twitter:title', content: `Bisanonton - ${name} - Videos` },
    ];
  },
);
