...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `Bisanonton - ${name} - Recommendations` },
      { property: 'og:title', content: `Bisanonton - ${name} - Recommendations` },
      {
        property: 'og:url',
        content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/recommendations`,
      },
      { property: 'twitter:title', content: `Bisanonton - ${name} - Recommendations` },
    ];
  },
);
