...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `Bisanonton - ${name}` },
      { property: 'og:title', content: `Bisanonton - ${name}` },
      { property: 'og:url', content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/` },
      { property: 'twitter:title', content: `Bisanonton - ${name}` },
    ];
  },
);
