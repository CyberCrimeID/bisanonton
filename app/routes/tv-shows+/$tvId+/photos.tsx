...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `Bisanonton - ${name} - Photos` },
      { property: 'og:title', content: `Bisanonton - ${name} - Photos` },
      {
        property: 'og:url',
        content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/photos`,
      },
      { property: 'twitter:title', content: `Bisanonton - ${name} - Photos` },
    ];
  },
);
