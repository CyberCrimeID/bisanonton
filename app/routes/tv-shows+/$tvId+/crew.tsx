...
export const meta = mergeMeta<typeof loader, { 'routes/tv-shows+/$tvId': typeof tvIdLoader }>(
  ({ matches, params }) => {
    ...
    const { name } = detail || {};
    return [
      { title: `Bisanonton - ${name} - Crew` },
      { property: 'og:title', content: `Bisanonton - ${name} - Crew` },
      { property: 'og:url', content: `${process.env.BASE_URL}/tv-shows/${params.tvId}/crew` },
      { property: 'twitter:title', content: `Bisanonton - ${name} - Crew` },
    ];
  },
);
