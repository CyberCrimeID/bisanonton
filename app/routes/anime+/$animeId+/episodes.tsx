...
export const meta = mergeMeta<{}, { 'routes/anime+/$animeId': typeof animeIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `Bisanonton - ${animeTitle} - Episodes` },
      {
        name: 'description',
        content: description
          ? description?.replace(/<\/?[^>]+(>|$)/g, '')
          : `Watch ${animeTitle} in Bisanonton`,
      },
      {
        property: 'og:url',
        content: `${process.env.BASE_URL}/anime/${params.animeId}/episodes`,
      },
      { property: 'og:title', content: `Bisanonton - ${animeTitle} - Episodes` },
      {
      ...
      { name: 'twitter:title', content: `Bisanonton - ${animeTitle} - Episodes` },
      {
