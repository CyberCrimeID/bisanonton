...
export const meta = mergeMeta<typeof loader>(({ data, params }) => {
  ...
  const { detail } = data;
  return [
    { title: `Bisanonton - ${detail?.name || ''}` },
    { name: 'description', content: `${detail?.description || ''}` },
    { name: 'keywords', content: `${detail?.name || ''}` },
    {
      property: 'og:url',
      content: `${process.env.BASE_URL}/lists/${params.listId}`,
    },
    { property: 'og:title', content: `Bisanonton - ${detail?.name || ''}` },
    { property: 'og:description', content: `${detail?.description || ''}` },
    { name: 'twitter:title', content: `Bisanonton - ${detail?.name || ''}` },
