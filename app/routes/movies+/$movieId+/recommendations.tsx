...
export const meta = mergeMeta<typeof loader, { 'routes/movies+/$movieId': typeof movieIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `Bisanonton - ${movieTitle} - Recommendations` },
      {
        property: 'og:url',
        content: `${process.env.BASE_URL}/movies/${params.movieId}/recommendations`,
      },
      { property: 'og:title', content: `Bisanonton - ${movieTitle} - Recommendations` },
      { name: 'twitter:title', content: `Bisanonton - ${movieTitle} - Recommendations` },
    ];
  },
);
