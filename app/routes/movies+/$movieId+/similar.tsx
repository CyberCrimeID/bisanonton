...
export const meta = mergeMeta<typeof loader, { 'routes/movies+/$movieId': typeof movieIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `Bisanonton - ${movieTitle} - Similar` },
      {
        property: 'og:url',
        content: `${process.env.BASE_URL}/movies/${params.movieId}/similar`,
      },
      { property: 'og:title', content: `Bisanonton - ${movieTitle} - Similar` },
      { name: 'twitter:title', content: `Bisanonton - ${movieTitle} - Similar` },
    ];
  },
);
