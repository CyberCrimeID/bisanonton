...
export const meta = mergeMeta<typeof loader, { 'routes/movies+/$movieId': typeof movieIdLoader }>(
  ({ params, matches }) => {
    ...
    return [
      { title: `Bisanonton - ${movieTitle} - Cast` },
      {
        property: 'og:url',
        content: `${process.env.BASE_URL}/movies/${params.movieId}/cast`,
      },
      { property: 'og:title', content: `Bisanonton - ${movieTitle} - Cast` },
      { name: 'twitter:title', content: `Bisanonton - ${movieTitle} - Cast` },
    ];
  },
);
