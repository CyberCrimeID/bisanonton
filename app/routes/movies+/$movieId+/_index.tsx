...
export const meta = mergeMeta<typeof loader, { 'routes/movies+/$movieId': typeof movieIdLoader }>(
  ({ matches, params }) => {
    ...
    const { title } = detail || {};
    return [
      { title: `Bisanonton - ${title}` },
      { property: 'og:title', content: `Bisanonton - ${title}` },
      { property: 'og:url', content: `${process.env.BASE_URL}/movies/${params.movieId}/` },
      { property: 'twitter:title', content: `Bisanonton - ${title}` },
    ];
  },
);
